...
func _on_movement_computed(computed_velocity):
	...
	if normal_velocity > 0.02:
		going = "right"
		#camera.position = base_cam_position
	elif normal_velocity < -0.02:
		going = "left"
		#camera.position = Vector3(-base_cam_position.x, base_cam_position.y, -base_cam_position.z)
	
	if going == "left":
		look_at(Vector3(cam.position.x, $Head.position.y, cam.position.z))
	else:
		look_at(position-Vector3(cam.position.x, $Head.position.y, cam.position.z))
